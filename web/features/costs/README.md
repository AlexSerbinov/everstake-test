...
Backend aggregation and its in-memory ledger regression tests live in `src/services/measurements/cost-dashboard.ts` and the adjacent test file. `public/costs.css` scopes the responsive mint/forest design. `cost-receipt.ts` remains the shared receipt component.
