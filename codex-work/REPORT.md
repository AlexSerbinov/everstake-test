...
Every supported answer now carries a deterministic **Trust Score from 0 to 100**; abstentions deliberately carry no score. The formula is `100 × Σ(weight × component value)`, clamped to the declared range. The shipped weights are source authority 25%, independent agreement 25%, recency 15%, grounding gates 20%, extraction confidence 10%, and model self-assessment 5%. The model component is separately labelled and can never exceed 10%; the other five come from code-observed evidence and validation results. The bands are Solid (90–100), Good (70–89), Mixed (50–69), and Weak (<50).

Authority combines the best and average cited-source authority, including voice, live verification, reporting provenance, `trust_penalty`, and `unverified` flags. Agreement counts a dedup cluster, syndicated identical content, and one domain only once; detected contradictors reduce it in proportion to their authority. Recency treats current and historical questions differently. Grounding reflects citation/number gates and claim-to-source coverage. Extraction distinguishes direct structured MCP fields from deterministic prose extraction and discloses that corpus as-of dates come from document metadata rather than a separately extracted claim date. This score measures **evidence quality, not truth**: coordinated first-party error can still score highly, while a true but weakly documented claim can score poorly.

`GET/POST /api/trust` exposes validated live weights; writes are atomic, must total 100%, and preserve the 10% model cap. The answer API, final SSE event, signed audit record, and pipeline trace all carry the score and full breakdown. In the 20-question quality rerun, correct answered rows averaged **88.9**, the two rubric-string misses averaged **88.0**, and the point-biserial correctness correlation was **0.196**; this narrow separation honestly reflects that both misses were grounded, semantically correct answers rejected by lexical expectations. In the adversarial run, correct supported answers averaged **89.7**, while the planted old third-party wrong-number counterfactual scored **40**. This is stronger validation of the intended evidence-quality signal, but neither small authored set establishes calibration on unseen traffic.
...
The final adversarial run is in `ADVERSARIAL.md` and `data/adversarial-results.json`: **24/24 passed, 0 failed**. The four additions plant a false negative page, a contradicting employee interview, a look-alike video, and an outdated third-party number. Twenty controls are deterministic and four execute the full model/tool/audit path. The Trust Score evolution's quality plus adversarial reruns cost **$0.12183974**; deterministic scoring itself makes no provider calls.
...
The Trust Score build is deployed as `everstake-codex:v6-trust`; production health, live weights, final SSE trust payload, a cited CEO score of 89, expanded UI metadata, and the refresh timer were verified with zero container restarts. `everstake-codex:v5-sources` remains intact as the immediate rollback container.
