...
## 2026-09-13 — Injection sanitizer coverage (branch `fix/injection-hardening`)

A read-only audit of `main` found the §5.4 boundary correctly placed (strip at ingestion, one JSON evidence channel, code gates) but narrow: English-only rules, unsanitized page titles and speaker fields, hidden inline-styled text reaching the sanitizer, one planted sentence deleting a whole unpunctuated transcript turn, and no planted-document test. Fixed in code: Unicode-aware rules for Ukrainian, Russian, German and Spanish; thirty-word windows for long unpunctuated segments; parenthetical directives cut without the host fact; `hidden`/`display:none`/zero-font/screen-reader-only elements dropped in the extractor; titles and speaker metadata pass `sanitizeField` before reaching the model; removed sentences stored on the snapshot as an audit trail (never model-visible). New tests: multilingual removal, ordinary UA/DE prose kept, transcript window, parenthetical, hidden text, title fallback, and an end-to-end planted-document test with a canary asserted absent from chunks, search and read. 158 tests pass. Measured on a copy of the live database (941 active documents, 8.8 M characters, already cleaned by the old rules): the new rules remove 0 additional sentences and alter 0 titles, so the added coverage costs no legitimate text; the existing audit trail shows 23 removals in 6 documents, 17 of them on `everstake.com/ai-info`. Stated limitation: hidden text from external stylesheets and directives in languages beyond these five still depend on the sentence rules; the model-based support review reads untrusted text and remains advisory-injectable.
