...
test('checks robots again after a redirect and refuses the disallowed path', async () => {
  ...
  const transport: FetchTransport = async url => {
    ...
    if (url.pathname === '/old') return response(url, 302, '', { location: '/private' });
    return response(url, 200, 'secret');
  };
  await assert.rejects(
    safeFetch('https://public.example/old', { transport, resolveHost: async () => ['93.184.216.34'], retries: 0 }),
    (error: unknown) => error instanceof CrawlFetchError && error.code === 'robots_disallowed',
  );
...
test('blocks private destinations before the transport runs', async () => {
  ...
  await assert.rejects(
    safeFetch('http://internal.example/admin', {
      ...
      transport: async url => { called = true; return response(url, 200, 'no'); },
    }),
    (error: unknown) => error instanceof CrawlFetchError && error.code === 'private_destination',
  );
...
test('refuses cross-origin redirects even when both hosts are public', async () => {
  ...
  await assert.rejects(
    safeFetch('https://public.example/page', { transport, resolveHost: async () => ['93.184.216.34'], retries: 0 }),
    (error: unknown) => error instanceof CrawlFetchError && error.code === 'cross_origin_redirect',
  );
});
...
test('retries a temporary HTTP failure within the configured bound', async () => {
  ...
  const transport: FetchTransport = async url => {
    ...
    pageAttempts += 1;
    return pageAttempts === 1 ? response(url, 503, 'busy') : response(url, 200, 'available');
  };
  const result = await safeFetch('https://public.example/page', { transport, resolveHost: async () => ['93.184.216.34'], retries: 1, retryDelayMs: 0 });
