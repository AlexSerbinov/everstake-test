...
type ExtendedSourceConfig = SourceConfig & {
  seedUrls?: string[];
  allowedHosts?: string[];
  includePaths?: string[];
  excludePaths?: string[];
...
function finaliseSanitation(
  ...
  return {
    ...
    metadata: {
      ...document.metadata,
      removedInstructionCount: sanitized.removed.length,
...
function record(
  ...
  ).run(url, status, reason, new Date().toISOString());
}
...
export async function crawlSource(
  ...
  if (source.kind === "youtube")
    throw new Error(
      "YouTube is handled by the dedicated video ingestion service",
    );
  const configured = source as ExtendedSourceConfig;
  const configuredSeeds = configured.seedUrls?.length
    ? configured.seedUrls
    : [source.url];
  const seeds = [
    ...new Set(
      (options.explicitUrls?.length
  ...
  const allowedHosts = new Set(
    (options.allowedHosts?.length
      ...
      : configured.allowedHosts?.length
  ...
  const fetchOptions = {
    ...
    robotsCache,
    rateLimitState: options.fetchOptions?.rateLimitState ?? {
  ...
  const permitted = (value: string): boolean => {
    ...
    return (
      ...
      includePaths.some((prefix) => url.pathname.startsWith(prefix)) &&
      !excludePaths.some((prefix) => url.pathname.startsWith(prefix))
    );
  };
  ...
  if (options.discoverSitemaps ?? expand) {
    const discovery = await discoverSitemaps(source.url, {
      ...fetchOptions,
      maxUrls: options.maxDiscoveredUrls ?? Math.max(maxPages * 20, 1_000),
    });
    for (const url of discovery.urls) enqueue(url, 0, "sitemap");
    for (const failure of discovery.errors)
      exclusions.push({
        url: failure.url,
        reason: "sitemap_failure",
    ...
    for (const result of results) {
      ...
      if ((options.discoverLinks ?? expand) && result.item.depth < maxDepth) {
        for (const link of result.extracted!.links)
          enqueue(link, result.item.depth + 1, result.item.url);
      }
    }
  }
  ...
  for (const url of pending)
    emit(
      url,
      "pending",
      documents.length >= maxPages ? "page_limit" : "byte_limit",
    );
  for (const candidate of candidates)
    emit(candidate.url, "candidate", "host_not_allowed");
  return {
    sourceId: source.id,
    documents,
    exclusions,
