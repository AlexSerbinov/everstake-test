...
const source: SourceConfig = { id: 'site', url: 'https://public.example', publisher: 'Example', authority: 1, kind: 'website', reason: 'Primary source', enabled: true };
...
test('keeps table context and distinguishes page dates from fetch time', () => {
  ...
  const document = extractDocument({ url: 'https://public.example/rates', initialUrl: 'https://public.example/rates', redirectChain: [], fetchedAt: '2026-09-13T12:00:00.000Z', bytes: html.length, status: 200, headers: { 'content-type': 'text/html' }, body: Buffer.from(html) }, source);
  ...
  assert.deepEqual(document.metadata.dateProvenance, { publishedAt: 'meta:article:published_time', updatedAt: 'jsonld:dateModified' });
...
test('does not treat migration headers as publication dates', () => {
  const html = '<article><h1>Archived article</h1><p>This article has enough original content to remain useful after a website migration without inventing a publication date.</p></article>';
  const document = extractDocument({ url: 'https://public.example/archive', initialUrl: 'https://public.example/archive', redirectChain: [], fetchedAt: '2026-09-13T12:00:00.000Z', bytes: html.length, status: 200, headers: { 'content-type': 'text/html', 'last-modified': 'Sun, 13 Sep 2026 10:00:00 GMT' }, body: Buffer.from(html) }, source);
...
test('extracts bounded Markdown sources without dropping tables', () => {
  ...
  const document = extractDocument({ url: 'https://public.example/README.md', initialUrl: 'https://public.example/README.md', redirectChain: [], fetchedAt: '2026-09-13T12:00:00.000Z', bytes: markdown.length, status: 200, headers: { 'content-type': 'text/markdown' }, body: Buffer.from(markdown) }, { ...source, kind: 'github' });
