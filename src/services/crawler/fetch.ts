...
export const CRAWLER_USER_AGENT =
  "EverstateKnowledgeCrawler/1.0 (+https://everstake.com)";
...
async function resolvePublicAddress(
  ...
  if (
    ...
    throw new CrawlFetchError(
      "unsafe_url",
      `Unsupported or credentialed URL: ${url.href}`,
    );
  }
  ...
  if (
    ...
    throw new CrawlFetchError(
      "private_destination",
      `Destination does not resolve exclusively to public IPs: ${url.hostname}`,
    );
  }
        ...
        response.on("end", () => {
          ...
          try {
            ...
            if (encoding === "gzip")
              ...
              body = inflateSync(body, { maxOutputLength: options.maxBytes });
            else if (encoding === "br")
          ...
          if (body.length > options.maxBytes) {
            reject(
              new CrawlFetchError(
                "oversized_response",
                `Decoded response exceeds ${options.maxBytes} bytes`,
              ),
            );
            return;
          }
...
async function waitForRequestSlot(
  ...
  await previous;
  const remaining =
    (state.lastStartedAt.get(origin) ?? 0) + minDelayMs - Date.now();
    ...
    try {
      ...
      const response = await options.transport(url, {
        ...
        headers: {
          ...
          accept:
            "text/html,text/markdown,text/plain,application/xml,text/xml;q=0.9,*/*;q=0.1",
      ...
      if (
        ...
        return response;
      lastError = new CrawlFetchError(
        "http_error",
        `HTTP ${response.status} for ${url.href}`,
      );
    } catch (error) {
      lastError = error;
      if (
        error instanceof CrawlFetchError &&
        ["private_destination", "unsafe_url", "oversized_response"].includes(
          error.code,
        )
      )
        throw error;
...
async function loadRobots(
  ...
  for (let redirects = 0; redirects <= options.maxRedirects; redirects += 1) {
    ...
    if (
      ...
      if (next.origin !== current.origin)
        throw new CrawlFetchError(
          "robots_unavailable",
          "Cross-origin robots redirect refused",
        );
      current = next;
      continue;
    }
    ...
    const result = {
      allowed: (url: string, userAgent: string) =>
        parsed.isAllowed(url, userAgent) !== false,
  ...
  throw new CrawlFetchError(
    "too_many_redirects",
    "robots.txt exceeded redirect limit",
  );
}
...
export async function safeFetch(
  ...
  for (let redirects = 0; redirects <= config.maxRedirects; redirects += 1) {
    ...
    if (!robots.allowed(current.href, config.userAgent))
      throw new CrawlFetchError(
        "robots_disallowed",
        `robots.txt disallows ${current.href}`,
      );
    ...
    if (
      ...
      if (next.origin !== current.origin)
        throw new CrawlFetchError(
          ...
          `Redirect leaves allowed origin: ${next.href}`,
        );
  ...
  throw new CrawlFetchError(
    "too_many_redirects",
    `Exceeded ${config.maxRedirects} redirects`,
  );
}
