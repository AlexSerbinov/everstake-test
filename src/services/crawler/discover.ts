...
import { discoverRobotsSitemaps, safeFetch, type SafeFetchOptions } from './fetch.js';
...
export async function discoverSitemaps(seedUrl: string, options: SafeFetchOptions & { maxSitemaps?: number; maxUrls?: number } = {}): Promise<SitemapDiscoveryResult> {
  const origin = new URL(seedUrl).origin;
  const declared = await discoverRobotsSitemaps(seedUrl, options).catch(() => []);
  const queue = [...new Set([...declared, new URL('/sitemap.xml', origin).href])];
  ...
  while (queue.length && visited.size < maxSitemaps) {
    ...
    if (visited.has(sitemapUrl) || new URL(sitemapUrl).origin !== origin) continue;
    visited.add(sitemapUrl);
    try {
      const response = await safeFetch(sitemapUrl, { ...options, maxBytes: Math.max(options.maxBytes ?? 0, 5_000_000) });
      ...
      errors.push({ url: sitemapUrl, reason: error instanceof Error ? error.message : String(error) });
    }
  }
