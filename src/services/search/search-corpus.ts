...
export function searchCorpus(db: Database, query: string, limit = 12): EvidencePassage[] {
 const words = [...new Set(query.match(/[\p{L}\p{N}]{2,}/gu) ?? [])].slice(0, 24);
 if (!words.length) return [];
 const expression = words.map(w => `"${w.replaceAll('"', '')}"`).join(' OR ');
 const rows = db.prepare(`SELECT c.id,c.document_id,c.text,bm25(chunks_fts) AS rank,d.snapshot
 FROM chunks_fts JOIN chunks c ON c.id=chunks_fts.id JOIN documents d ON d.id=c.document_id
 WHERE chunks_fts MATCH ? AND d.active=1 ORDER BY rank LIMIT 100`).all(expression) as unknown as {id:string;document_id:string;text:string;rank:number;snapshot:string}[];
 ...
 for (const row of rows) {
  ...
  results.push(passage(document, row.id, row.text, -row.rank, 'Lexical match; at most two passages per duplicate group'));
