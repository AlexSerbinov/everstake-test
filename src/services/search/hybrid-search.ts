...
export async function hybridSearch(
 ...
 JOIN documents d ON d.id=c.document_id WHERE d.active=1 AND e.model='text-embedding-3-small'`,
    )
    .all() as { id: string; text: string; vector: string; snapshot: string }[];
  ...
  for (const row of [...merged.values()].sort((a, b) => b.rank - a.rank)) {
    ...
    if ((groups.get(e.duplicateGroup) ?? 0) >= 2) continue;
