...
export function beginRun(
  ...
  const id = randomUUID();
  const parentRunId = typeof metadata.parentRunId === 'string' ? metadata.parentRunId : currentRunId();
  const persistedMetadata = parentRunId ? { ...metadata, parentRunId } : metadata;
  db.prepare(
    'INSERT INTO runs(id, kind, started_at, finished_at, status, metadata) VALUES (?, ?, ?, NULL, ?, ?)',
  ).run(id, kind, new Date().toISOString(), 'running', JSON.stringify(persistedMetadata));
  activeMeasurements.set(id, { cpu: process.cpuUsage(), peakRssBytes: process.resourceUsage().maxRSS * 1024 });
  return id;
}

export function finishRun(db: Database, id: string, status: RunStatus): void {
  if (status === 'running') throw new Error('A finished run cannot have status running');
  const row = db.prepare('SELECT metadata, status FROM runs WHERE id = ?').get(id) as {
    metadata: string; status: string;
  } | undefined;
  if (!row) throw new Error(`Unknown run: ${id}`);
  if (row.status !== 'running') throw new Error(`Run is already finished: ${id}`);
  ...
  const measurement = {
    ...
    peakRssBytes: active ? Math.max(active.peakRssBytes, peakRssBytes) : peakRssBytes,
    resourceScope: 'process',
  };
  db.prepare("UPDATE runs SET finished_at = ?, status = ?, metadata = ? WHERE id = ? AND status = 'running'").run(
