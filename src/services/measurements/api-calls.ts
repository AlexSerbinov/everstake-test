...
export function beginApiAttempt(
  ...
  if (!Number.isFinite(input.reservationUsd) || input.reservationUsd < 0) {
    throw new Error(
      "API budget reservation must be a non-negative finite number",
    );
  }
  const id = randomUUID();
...
export function finishApiAttempt(
  ...
  const row = db
    .prepare("SELECT model, metadata FROM api_calls WHERE id = ?")
    .get(id) as
    | {
        model: string;
        metadata: string;
  ...
  const costUsd =
    result.actualCostUsd !== undefined
      ...
      : result.usage && result.price
        ? calculateCost(result.usage, result.price)
        : null;
  const safeError =
    result.error === undefined ? null : sanitizeError(result.error);
  const metadata = {
    ...prior,
    ...result.metadata,
  ...
  if (changed.changes !== 1)
    throw new Error(`API attempt is already finished: ${id}`);
}
...
export function sanitizeError(error: unknown): string {
  ...
  return message
    ...
    .replace(
      /\b(?:AIza[\w-]{20,}|sk-[\w-]{12,}|Bearer\s+[^\s]+)/gi,
      "[redacted]",
    )
    .replace(
...
function enforceBudget(
  ...
  if (
    session.unknownUnreserved ||
    session.usedUsd + reservationUsd > caps.sessionUsd
  ) {
...
function budgetUse(
  ...
  where: string,
  values: string[],
): { usedUsd: number; unknownUnreserved: boolean } {
  const rows = db
    .prepare(`SELECT cost_usd, metadata FROM api_calls WHERE ${where}`)
    .all(...values) as Array<{
...
function validCost(value: number | null): number | null {
  ...
  if (!Number.isFinite(value) || value < 0)
    throw new Error("API cost must be a non-negative finite number or null");
  return value;
}
