...
export function costOverview(db: Database): CostOverview {
  const rows = db
    .prepare("SELECT * FROM api_calls ORDER BY started_at, rowid")
  ...
  return {
    ...
    byModel: group(rows, (row) => `${row.provider}\u0000${row.model}`).map(
      ([key, groupRows]) => {
        const [provider, model] = key.split("\u0000");
        return summary(groupRows, { provider, model });
      },
    ),
    byStatus: group(rows, (row) => row.status).map(([status, groupRows]) =>
      summary(groupRows, { status }),
    ),
  };
}

function descendantRunIds(db: Database, rootId: string): string[] {
  const rows = db.prepare("SELECT id, metadata FROM runs").all() as Array<{
    id: string;
    metadata: string;
  }>;
...
function summary<T extends Record<string, string>>(
  rows: CallRow[],
  labels: T,
): T & {
  calls: number;
...
function group(
  rows: CallRow[],
  key: (row: CallRow) => string,
): Array<[string, CallRow[]]> {
...
function sumKnown(
  rows: CallRow[],
  key: "input_tokens" | "output_tokens" | "cost_usd",
): number {
