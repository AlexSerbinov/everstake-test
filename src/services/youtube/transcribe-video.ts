...
export async function transcribeVideo(
  ...
  const row = db
    ...
    .get(videoId) as { data: string } | undefined;
  ...
  const persist = (): void => {
    db.prepare(
      "INSERT OR REPLACE INTO youtube_jobs(video_id,status,data) VALUES(?,?,?)",
    ).run(videoId, job.status, JSON.stringify(job));
    mkdirSync(artifactDirectory, { recursive: true });
    writeFileSync(
      join(artifactDirectory, `${videoId}.job.json`),
  ...
  if (job.status === "submission_unknown") {
    const operationId =
      typeof job.providerMetadata?.operationId === "string"
        ? job.providerMetadata.operationId
        : null;
    const reconciled =
      operationId && transport.findByClientReference
        ? await transport.findByClientReference(operationId)
        : null;
    if (!reconciled)
      throw new Error(
        "Prior Soniox submission is ambiguous; reconcile provider activity before resubmitting",
      );
    job.transcriptionId = reconciled.id;
    job.status = "submitted";
    job.providerMetadata = {
      ...job.providerMetadata,
      reconciledByClientReference: true,
  ...
  let attemptId =
    typeof job.providerMetadata?.attemptId === "string"
      ? job.providerMetadata.attemptId
      : undefined;
  ...
  if (!attemptId)
    throw new Error("Submitted Soniox job has no metered attempt ID");
  ...
  while (Date.now() < deadline) {
    ...
    if (providerStatus === "completed") {
      ...
      mkdirSync(artifactDirectory, { recursive: true });
      writeFileSync(
        join(artifactDirectory, `${videoId}.transcript.json`),
    ...
    if (["error", "failed", "canceled", "cancelled"].includes(providerStatus)) {
      ...
      throw new Error(
        `Soniox transcription failed with status ${providerStatus}`,
      );
    }
...
export function createSonioxTransport(
  ...
  const request = async (
    ...
    if (!response.ok)
      throw new Error(
        `Soniox HTTP ${response.status}: ${String(body.error_message ?? body.message ?? "request failed").slice(0, 300)}`,
      );
  ...
  return {
    async upload(audioPath, bytes) {
      ...
      return {
        id: requiredId(response.body, "upload"),
    ...
    async submit(fileId, model, clientReferenceId) {
      ...
      return {
        id: requiredId(response.body, "transcription"),
        requestId: response.requestId,
      };
    },
    async findByClientReference(clientReferenceId) {
      const response = await request("/transcriptions?limit=1000");
      const rows = Array.isArray(response.body.transcriptions)
        ? response.body.transcriptions
        : [];
      const match = rows.find(
        (item) =>
          item &&
          typeof item === "object" &&
          (item as Record<string, unknown>).client_reference_id ===
            clientReferenceId,
    ...
    async status(transcriptionId) {
      return (await request(`/transcriptions/${transcriptionId}`)).body;
    },
    async transcript(transcriptionId) {
      return (await request(`/transcriptions/${transcriptionId}/transcript`))
        .body;
    },
  };
}
...
function requiredId(body: Record<string, unknown>, operation: string): string {
  if (typeof body.id !== "string" || !body.id)
    throw new Error(`Invalid Soniox ${operation} response`);
  return body.id;
}
