...
test('unknown duration cannot be accepted for paid processing',()=>{const r=screenVideo({id:'unknown-time',title:'Everstake staking',channelId:'official',url:'https://youtube.com/watch?v=x',publishedAt:null,durationSeconds:null},['official']);assert.equal(r.decision,'needs_review');assert.match(r.reason,/Duration is unknown/);});
