...
import { screenVideo, type ScreeningPolicy, type VideoCandidate } from './screen-videos.js';
...
export interface Inventory {
  generatedAt: string;
  sources: Array<{ kind: string; location: string; checkedAt: string; note: string }>;
...
export function importInventory(options: {
  ...
  const add = (input: Omit<VideoCandidate, 'decision' | 'reason'>, evidence: Record<string, unknown>): void => {
    const prior = merged.get(input.id);
    const provenance = [...(prior?.provenance ?? []), evidence];
    merged.set(input.id, {
      ...(prior ?? input),
      ...Object.fromEntries(Object.entries(input).filter(([, value]) => value !== '' && value !== null)),
      publishedAt: input.publishedAt ?? prior?.publishedAt ?? null,
      durationSeconds: input.durationSeconds ?? prior?.durationSeconds ?? null,
      provenance,
      metadata: { ...prior?.metadata, ...input.metadata, cacheOnly: true, requiresFreshAudio: true },
    });
  };
  ...
  for (const item of old.candidates ?? []) {
    ...
    const info = existsSync(infoPath) ? JSON.parse(readFileSync(infoPath, 'utf8')) as Record<string, unknown> : {};
  ...
  for (const line of readFileSync(options.liveFlatList, 'utf8').split(/\r?\n/).filter(Boolean)) {
    const item = JSON.parse(line) as Record<string, unknown>;
    const id = String(item.id ?? '');
    if (!/^[\w-]{11}$/.test(id)) continue;
    const candidate = candidateFromRecords(id, item, {});
    candidate.channelId = config.officialChannel.id;
    candidate.channel ||= stringValue(item.channel ?? item.uploader) || 'Everstake';
    add(candidate, {
      kind: 'live-official-channel-flat-list', location: options.liveFlatList, checkedAt,
      channelHandle: config.officialChannel.handle,
    });
  }

  const policy: ScreeningPolicy = { ...config.screening, officialChannelIds: [config.officialChannel.id] };
  ...
  const counts = Object.fromEntries(['accepted', 'needs_review', 'excluded'].map(decision => [
    decision, candidates.filter(candidate => candidate.decision === decision).length,
  ]));
  const accepted = candidates.filter(candidate => candidate.decision === 'accepted');
  const inventory: Inventory = {
    generatedAt: checkedAt,
    sources: [
      { kind: 'legacy-manifest', location: options.oldManifest, checkedAt, note: 'Imported as candidate provenance and re-screened; prior outcomes were not silently accepted.' },
      { kind: 'live-channel-flat-list', location: options.liveFlatList, checkedAt, note: 'Fresh metadata inventory; full metadata and audio are fetched only for accepted processing.' },
    ],
    counts: { discovered: candidates.length, ...counts },
    acceptedDurationSeconds: accepted.reduce((sum, candidate) => sum + (candidate.durationSeconds ?? 0), 0),
    unknownAcceptedDurations: accepted.filter(candidate => candidate.durationSeconds === null).length,
    candidates,
  };
...
function candidateFromRecords(id: string, item: Record<string, unknown>, info: Record<string, unknown>): Omit<VideoCandidate, 'decision' | 'reason'> {
  ...
  return {
    ...
    publishedAt: uploadDate ? /^\d{8}$/.test(uploadDate) ? `${uploadDate.slice(0, 4)}-${uploadDate.slice(4, 6)}-${uploadDate.slice(6)}` : uploadDate : null,
    durationSeconds: numberValue(info.duration ?? item.duration_sec ?? item.duration),
    provenance: [],
    metadata: {
      captionLanguages: Object.keys(recordValue(info.subtitles)),
      automaticCaptionLanguages: Object.keys(recordValue(info.automatic_captions)),
      recordingDate: stringValue(info.release_date) || null,
    },
  };
}

function remember(current: Omit<VideoCandidate, 'decision' | 'reason'>, infoPath: string): void {
  current.provenance = [...(current.provenance ?? []), { kind: 'legacy-yt-dlp-info', location: infoPath }];
}
...
export async function discoverOfficialChannel(output: string): Promise<void> {
  ...
  const { stdout } = await exec(binary, ['--flat-playlist', '--dump-json', '--no-warnings', config.officialChannel.handle], { maxBuffer: 32 * 1024 * 1024 });
  writeFileSync(output, stdout);
}

export async function downloadAcceptedAudio(candidate: VideoCandidate, outputDirectory: string): Promise<string> {
  if (candidate.decision !== 'accepted') throw new Error(`Refusing paid processing for ${candidate.id}: ${candidate.decision}`);
  mkdirSync(outputDirectory, { recursive: true });
  const output = join(outputDirectory, `${candidate.id}.m4a`);
  if (existsSync(output)) return output;
  const args = ['--no-playlist', '--no-warnings', '-f', 'bestaudio[ext=m4a]/bestaudio', '-o', output];
  if (process.env.YT_PROXY) args.push('--proxy', process.env.YT_PROXY);
  const cookieCopy = process.env.YT_COOKIES_FILE ? join(outputDirectory, `.cookies-${process.pid}.txt`) : null;
  if (cookieCopy) {
    copyFileSync(process.env.YT_COOKIES_FILE!, cookieCopy);
    args.push('--cookies', cookieCopy);
  }
  if (process.env.YT_JS_RUNTIMES) args.push('--js-runtimes', process.env.YT_JS_RUNTIMES);
  args.push(candidate.url);
  try {
    await exec(process.env.YT_DLP_BIN ?? 'yt-dlp', args, { timeout: 30 * 60 * 1_000, maxBuffer: 8 * 1024 * 1024 });
  } finally {
...
export function buildDocument(candidate: VideoCandidate, turns: Turn[], review: SpeakerReview, fetchedAt = new Date().toISOString()): DocumentSnapshot {
  if (candidate.decision !== 'accepted') throw new Error(`Cannot emit excluded video ${candidate.id}`);
  const speakers = new Map(review.speakers.map(speaker => [speaker.label, speaker]));
  const lines = turns.map(turn => {
    const reviewed = turn.speaker === null ? null : speakers.get(turn.speaker);
    const identity = reviewed?.name ?? (turn.speaker === null ? 'Unknown speaker' : `Speaker ${turn.speaker}`);
    const role = reviewed?.roleAtRecording ? `, ${reviewed.roleAtRecording}` : '';
