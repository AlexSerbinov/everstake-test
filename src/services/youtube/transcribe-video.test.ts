...
import {
  transcribeVideo,
  type SonioxTransport,
  type TranscriptionMeter,
} from "./transcribe-video.js";
...
test("rerun resumes a submitted provider job without a second paid submission", async () => {
  ...
  const transport: SonioxTransport = {
    async upload() {
      uploads += 1;
      return { id: "file-1", requestId: "upload-request" };
    },
    async submit() {
      submissions += 1;
      return { id: "transcription-1", requestId: "submit-request" };
    },
    async status() {
      return { status: "completed", request_id: "status-request" };
    },
    async transcript() {
      return {
        tokens: [{ text: "Hello", speaker: "1", start_ms: 0, end_ms: 500 }],
      };
    },
  };
  ...
  await assert.rejects(
    transcribeVideo(db, "DFSEQ3OGoL8", audio, meter, {
      transport,
      pollTimeoutMs: 0,
