...
export function screenVideo(
  ...
  if (lookalike)
    return {
      ...candidate,
      decision: "excluded",
      reason: `Metadata matches configured look-alike term "${lookalike}"; name similarity is not company identity.`,
    };
  if (
    ...
    return {
      ...
      reason:
        "Duration is unknown or invalid; paid processing requires a fresh positive duration.",
    };
  }
  if (
    candidate.durationSeconds !== null &&
    ...
    return {
      ...candidate,
      decision: "needs_review",
      reason: `Duration is ${candidate.durationSeconds}s, below the ${policy.minimumDurationSeconds}s screening floor; inspect before paid transcription.`,
    };
  }
  ...
  if (company && topic && participation) {
    return {
      ...candidate,
      decision: "accepted",
      reason: `Third-party metadata names the company, topic "${topic}" and participation signal "${participation}"; transcript review must verify speakers and claims.`,
    };
  }
  if (company && topic)
    return {
      ...
      decision: "needs_review",
      reason:
        "Company and topic match, but metadata does not establish direct participation; inspect captions or the recording.",
    };
  return {
    ...candidate,
    decision: "excluded",
    reason:
      "Metadata contains neither a configured company-and-topic match nor an official publisher identity.",
  };
}
