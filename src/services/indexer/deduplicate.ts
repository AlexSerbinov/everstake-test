  ...
  for (const document of input) {
    ...
    if (!existing) {
      const {
        ...
        duplicateReason: _reason,
        duplicateSimilarity: _similarity,
        ...metadata
      } = document.metadata;
  ...
  for (const indexes of members.values()) {
    ...
    for (const document of grouped) {
      ...
      document.metadata = {
        ...document.metadata,
        duplicateGroup: id,
        duplicateReason: exact ? "exact_content" : "near_duplicate",
    ...
    groups.push({
      ...
      documentIds: grouped.map((document) => document.id),
      reason: exact ? "exact_content" : "near_duplicate",
      similarity,
    });
  }
  for (const document of documents) {
    if (!document.metadata.duplicateGroup)
      document.metadata = { ...document.metadata, duplicateGroup: document.id };
  }
