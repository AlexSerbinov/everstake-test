...
export function buildIndex(
  ...
  const indexableDocuments = deduplicated.documents.filter(
    (document) =>
      ...
      document.metadata.duplicateReason === "near_duplicate",
  );
  ...
  const deactivateAll = db.prepare(
    "UPDATE documents SET active=0 WHERE active=1",
  );
  ...
  return {
    version,
    documents: deduplicated.documents.length,
    canonicalDocuments: representatives.length,
    chunks: chunks.length,
    duplicateGroups: deduplicated.groups,
  };
}
