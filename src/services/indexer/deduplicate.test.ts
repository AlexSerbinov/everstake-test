...
test("groups exact and near copies but keeps every original snapshot", () => {
  const base =
    "Everstake provides secure non-custodial staking for institutions and token holders across public proof of stake networks with transparent infrastructure operations.";
  ...
  assert.equal(
    result.documents.filter((item) => item.duplicateOf === null).length,
    1,
  );
});
...
test("keeps revised facts on the same canonical URL separate", () => {
  const result = deduplicateDocuments([
    document(
      ...
      "The service supports 70 networks.",
      "old",
    ),
    document(
      "new",
      "https://example.com/fact",
      "The service supports 90 networks.",
      "new",
    ),
  ]);
...
test("regrouping clears references to a missing previous representative", () => {
  const retained = {
    ...document(
      ...
      "The retained original content remains searchable.",
    ),
    duplicateOf: "gone",
  };
  const result = deduplicateDocuments([retained]);
  assert.equal(result.documents[0]!.duplicateOf, null);
});
