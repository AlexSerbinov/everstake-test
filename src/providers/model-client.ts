...
import { openAiGenerationRequest, parseOpenAiEmbeddings, parseOpenAiGeneration, providerError } from './openai.js';
import {
  apiKey,
  geminiProtocol,
  loadModelsConfig,
...
export function createModelClient(db: Database, options: ClientOptions = {}): ModelClient {
  ...
  return {
    generate: request => runMeteredAttempts(db, common, {
      request,
      provider: 'gemini',
      ...
      call: async context => {
        ...
        if (protocol === 'native') {
          ...
          if (!parsed.text) {
            throw new MeteredResponseError(
              'Gemini returned no text', parsed.model, parsed.usage,
              parsed.providerRequestId ?? response.headers.get('x-request-id'),
            );
          }
          return { value: parsed, model: parsed.model, usage: parsed.usage, requestId: parsed.providerRequestId ?? response.headers.get('x-request-id') };
        }
        const response = await common.fetch(`${context.baseURL}/chat/completions`, {
          method: 'POST', signal: context.signal,
          headers: { 'content-type': 'application/json', authorization: `Bearer ${context.key}` },
        ...
        if (!parsed.text) {
          throw new MeteredResponseError(
            'Provider returned no text', parsed.model, parsed.usage,
            parsed.providerRequestId ?? response.headers.get('x-request-id'),
          );
        }
        return { value: parsed, model: parsed.model, usage: parsed.usage, requestId: parsed.providerRequestId ?? response.headers.get('x-request-id') };
...
export function createEmbeddingClient(db: Database, options: ClientOptions = {}): EmbeddingClient {
  const common = clientContext(options);
  const settings = common.config.providers.openai;
  return {
    embed: request => runMeteredAttempts(db, common, {
      request,
      provider: 'openai',
      ...
      call: async context => {
        ...
        const response = await common.fetch(`${context.baseURL}/embeddings`, {
          method: 'POST', signal: context.signal,
          headers: { 'content-type': 'application/json', authorization: `Bearer ${context.key}` },
          body: JSON.stringify({ model, input: request.input, encoding_format: 'float' }),
        });
        const body = await responseBody(response);
        const bodyRecord = asRecord(body);
        const actualModel = typeof bodyRecord.model === 'string' ? bodyRecord.model : model;
        ...
        return { value: parsed, model: parsed.model, usage: parsed.usage, requestId: response.headers.get('x-request-id') };
      },
      response: result => ({
...
function clientContext(options: ClientOptions) {
  const policy = readConfig<{ maxRunCostUsd: number; maxSessionCostUsd: number }>('policy');
  return {
...
async function runMeteredAttempts<T, R>(
  ...
  const baseURL = providerBaseUrl(options.provider as 'gemini' | 'openai', options.settings, common.environment);
  ...
  for (let attempt = 1; attempt <= options.settings.maxAttempts; attempt += 1) {
    ...
    const timer = setTimeout(() => controller.abort(), options.settings.timeoutMs);
    ...
    try {
      ...
      finishApiAttempt(db, attemptId, {
        ...
        price: result || metered ? priceFor(common.config, result?.model ?? metered!.model) : null,
...
function retryable(error: unknown, timedOut: boolean): boolean {
  ...
  const match = error instanceof Error ? /^HTTP (\d+):/.exec(error.message) : null;
