...
export function nativeGeminiRequest(request: ModelRequest, model: string): Record<string, unknown> {
  return {
    systemInstruction: { parts: [{ text: request.system }] },
    contents: request.messages.map(message => ({ role: message.role, parts: [{ text: message.text }] })),
    generationConfig: request.maxOutputTokens === undefined
      ? {}
      : { maxOutputTokens: request.maxOutputTokens },
  };
}

export function parseNativeGemini(body: unknown, requestedModel: string): ParsedGeneration {
