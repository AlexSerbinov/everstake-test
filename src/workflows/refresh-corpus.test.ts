...
function prior(): DocumentSnapshot {
  return {
    ...
    authority: 1,
    kind: "website",
    text: "The old but still usable fact remains available as evidence until a safe replacement succeeds.",
    contentHash: "old",
...
test("failed targeted refresh leaves the prior active corpus unchanged", async () => {
  ...
  const report = await refreshCorpus(db, [source], {
    version: "v2",
    crawl: {
...
test("successful targeted refresh stores the old revision but activates the replacement", async () => {
  ...
  buildIndex(db, [prior()], { version: "v1" });
  const body =
    "<main><h1>Current fact</h1><p>The newly verified source now contains a materially changed fact with its full conditions and context for readers.</p></main>";
  const transport: FetchTransport = async (url) =>
  ...
  const rows = [
    ...db.prepare("SELECT id,active FROM documents ORDER BY active,id").all(),
  ].map((row) => ({ ...row })) as Array<{ id: string; active: number }>;
