...
export interface BuildCorpusOptions {
  ...
  sourceOptions?:
    | Record<string, CrawlSourceOptions>
    | ((source: SourceConfig) => CrawlSourceOptions);
  version?: string;
...
function optionsFor(
  ...
  const sourceOptions =
    typeof options.sourceOptions === "function"
...
export function crawlOptionsFor(
  source: SourceConfig,
  options: BuildCorpusOptions,
): CrawlSourceOptions {
...
export function readActiveDocuments(db: Database): DocumentSnapshot[] {
  return (
    db.prepare("SELECT snapshot FROM documents WHERE active=1").all() as Array<{
      snapshot: string;
    }>
  ).map((row) => JSON.parse(row.snapshot) as DocumentSnapshot);
}
...
export async function buildCorpus(
  ...
  if (documents.length === 0)
    throw new Error(
      "Corpus build produced no accepted documents; current index was not changed",
    );
